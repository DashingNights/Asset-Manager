...
router.get("/inventory/editpc/new", async (req, res, next) => {
	res.render("main/data-editing/new/newPC", { pc: new PcInventory() });
});
router.post("/inventory/editpc/new", async (req, res, next) => {
...
router.get("/inventory/editmonitor/new", async (req, res, next) => {
	res.render("main/data-editing/new/newMonitor", { monitor: new MonitorInventory() });
});
router.post("/inventory/editmonitor/new", async (req, res, next) => {
		...
		switch (savetype) {
			case "user": {
				...
				try {
					...
					if (user) {
						res.send("New user " + user.email + " with name " + user.Item_Holder);
					}
				} catch (err) {
					console.error(err);
			...
			case "pc": {
				...
				try {
					...
					if (pc) {
						res.send("New pc " + pc.PC_ID + " with serial " + pc.Serial_Number);
					}
				} catch (err) {
					console.error(err);
					res.send("Error updating item");
		...
		switch (savetype) {
			case "user": {
				...
				try {
					...
					if (user) {
						res.send("Updated user " + user.email + " with name " + user.Item_Holder);
					} else {
			...
			case "pc": {
				...
				try {
					...
					if (pc) {
						res.send("Updated pc " + pc.PC_ID + " with serial " + pc.Serial_Number);
					} else {
						res.send("No pc found with the provided id");
					}
				} catch (err) {
					console.error(err);
					res.send("Error updating item");
			...
			case "monitor": {
				const monitorData = {
					...
					Price: req.body.Price,
					Dates: {
						Purchased: req.body.Dates.Purchased,
						Warranty_End: req.body.Dates.Warranty_End,
					},
					Model: req.body.Model,
					Serial_Number: req.body.Serial_Number,
					Asset_ID: req.body.Asset_ID,
				...
				try {
					...
					if (pc) {
						res.send("Updated pc " + monitor.Monitor_ID + " with serial " + monitor.Serial_Number);
					} else {
						res.send("No monitor found with the provided id");
					}
				} catch (err) {
					console.error(err);
					res.send("Error updating item");
